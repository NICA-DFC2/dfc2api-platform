...
' );
    ...
    private function createCategories(OutputInterface $output){
        ...
        foreach ($dumpedCategories as $dumpedCategory){
            $category = new Category();
            $category->setOldId ($dumpedCategory['id']);
            $category->setOldIdParent ($dumpedCategory['idParent']);
            $category->setName ($dumpedCategory['name']);
            $em->persist($category);
            $progress->advance(1);
        }
        $em->flush();
    ...
    private function organizeCategory(OutputInterface $output){
         ...
         */
        $repository = $em->getRepository("App:Category");
